...
Template.boardHeaderBar.BlazeComponent({
  watchLevel() {
    const currentBoard = Boards.findOne(Session.get('currentBoard'));
  ...
  onCreated() {
    this.minifiedEditors = new ReactiveVar(false);
  },
  ...
  events() {
    return [{
      ...
      'click .js-change-list-width'(evt) {
        //Sidebar.setView('filter');
        evt.stopPropagation();
        var m = State.isInSortMode.get();
        if (m)
          State.isInSortMode.set(false);
        else
          State.isInSortMode.set(true);
      },
