...
Cards.helpers({
  ...
  getPrevious() {
    if (this.previousCardId) {
      return Cards.findOne(this.previousCardId);
    }
    else {
      return null;
    }
  ...
  getNext() {
    if (this.nextCardId) {
      return Cards.findOne(this.nextCardId);
    }
    else {
      return null;
    }
  ...
  getEntireDocument() {
    var entireDocument = this.segment;
    if (!entireDocument)
      return;

    var p = Cards.findOne(this.previousCardId);

    if (p && p.previousCardId) {
      if (p.segment)
        entireDocument = p.segment + entireDocument;
      var p = Cards.findOne(p.previousCardId);
      while (p && p.previousCardId) {
        if (p.segment)
          entireDocument = p.segment + entireDocument;
        var p = Cards.findOne(p.previousCardId);
      }
    ...
    var n = Cards.findOne(this.nextCardId);
    if (n && n.nextCardId) {
      if (n.segment)
        entireDocument = entireDocument + n.segment;
      var n = Cards.findOne(n.nextCardId);
      while (n && n.nextCardId) {
        if (n.segment)
          entireDocument = entireDocument + n.segment;
        var n = Cards.findOne(n.nextCardId);
      }
